...
def generate_observations(
    ...
    prompt = """Create one coherent set of entirely fictional adult nursing observations for education. Keep every value close to the supplied educator-authored baseline and consistent with the visible context. Do not diagnose, recommend treatment, add oxygen, or change alertness without support. If deterioration_stage is above zero, do not improve respiratory rate, oxygen saturation, or systolic blood pressure relative to baseline. Return only the schema-defined JSON.
