...
DEFAULT_LOCAL_LIBRARY = (
    Path(__file__).resolve().parents[1] / "sample_patients" / "patient_cases.json"
)
...
def _read_source(source: Path | str, token: str, timeout: int) -> bytes:
    ...
    if source_text.casefold().startswith(("http://", "https://")):
        ...
        headers = {"Accept": "application/json", "User-Agent": "Nurse-Simulation-Studio"}
        if token:
...
def _validate_case(case: Any, index: int) -> list[str]:
    ...
    if not isinstance(patient, dict):
        errors.append(f"{prefix}: patient must be an object")
        patient = {}
    ...
    if not patient.get("nonverbal_palette"):
        errors.append(f"{prefix}: patient needs a nonverbal palette")
